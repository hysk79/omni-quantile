...
% \end{align}
% where $S_\alpha$ is the class of \textit{all} scoring functions that are consistent for the $\alpha$-level quantile functional and $\cf$ is a set of competitors. Competitors can also viewed as base forecasters that provide $\alpha$-level quantile forecasts. Then, the predictions $\hat{P}_t$ can be viewed as (randomized) $\alpha$-level quantile forecast that minimizes all consistent scoring functions, with respect to given base forecasters $\cf$. This can also be viewed as an implicit ensemble of base forecasters $\cf$.
...
% While Algorithm \ref{alg:omni-twoplayer} takes the best competitor for each elementary score $S_\theta$ as input, Algorithm \ref{alg:omni-twoplayer_online_q} takes all predictions made by competitors as input, and the best competitor for each elementary score $S_{\alpha, i}$ is discovered and updated inside the algorithm. This removes the requirement of additional hold-out set for selecting the best competitor. To do so, Algorithm \ref{alg:omni-twoplayer_online_q} considers the set of competitors $\cf=\{f_b\}_{b=1}^F$ to be finite. That said, the algorithm can be easily modified to a version that accommodates more general $\cf$, that takes the best competitor for each elementary score $S_{\alpha, i}$ pre-computed outside the algorithm. Details can be found in Appendix \ref{sec:diff_omni_algos}.
...
\begin{algorithm}
  ...
  \[
  w_{ib}(t+1) \propto w_{ib}(t) \exp\bigl(\eta \big(\mathbb{E}_{p \sim \hat{P}_t}[S_{\alpha, i}(p, Y_t)] 
...
In subsequent sections, we use $S_\alpha$ to denote the class of scoring functions with representation \eqref{eq:elem_score_q} with $H(\R)=1$.
...
The first part of the algorithm is to make a prediction at time $t$. $\hat{P}_t$ computed here is a minimax solution for the following minimax program:
\[
...
\begin{theorem}
  ...
  For a quantile level $\alpha \in (0,1)$, let $S_\alpha$ denote the class of scoring functions with representation \eqref{eq:elem_score_q} with $H(\R) = 1$ (the set of \textit{all} scoring functions that are consistent for the $\alpha$-level quantile functional under mild regularity conditions). Consider a base forecasters $\{f_b\}_{b=1}^B$ provides $\alpha$-level quantile forecasts. With the target values $Y_t$, forecaster predictions $f_b(t)$ for $t \in [T], b \in [B]$ each taking values in a uniform grid of size $m$ with spacing $B/m$ and the learning rate $\eta = \sqrt{\frac{\log mB}{T}}$, the output $\hat{P}_t$ from Algorithm \ref{alg:omni-twoplayer_online_q} achieves the following omniprediction error:
  \begin{align*}
  \sup_{S \in S_\alpha,\, b \in [B]} \frac{1}{T} \sum_{t=1}^T \E_{p \sim \hat{P}_t} [S(p, Y_t)] - \E[S(f_b(t), Y_t)]
...
Regret bound of hedge algorithm (Theorem 1.5 of \cite{hazanoco}) gives the following:
\begin{align}
  \max_{i \in [m], b \in [B]} \frac{1}{T} \sum_{t=1}^T \sum_{b=1}^B 
  & \E_{p\sim \hat{P}_t}\left[ S_{\alpha, i}(p, Y_t) - S_{\alpha, i}(f_b(t), Y_t) \right] \nonumber\\
  & \leq \frac{1}{T} \sum_{t=1}^T \sum_{i=1}^{m} \sum_{b=1}^B w_{ib}(t) \, \E_{p\sim \hat{P}_t}\left[ S_{\alpha, i}(p, Y_t) - S_{\alpha, i}(f_b(t), Y_t) \right] + \eta + \frac{\log mB}{T\eta} \label{ieq:hedge_w_update}
\end{align}
...
Putting these together with previous lemmas, we have:
  \begin{align*}
    ...
    & \underset{\text{Lem. } \ref{lem:discretize_single_q}}{=} \sup_{i \in [m], b \in [B]} \frac{1}{T} \sum_{t=1}^T \E_{p \sim \hat{P}_t} [S_{\alpha, i}(p, Y_t)] - S_{\alpha, i}(f(t), Y_t) \\
    & \underset{Ieq. \eqref{ieq:hedge_w_update}}{\leq} \frac{1}{T} \sum_{t=1}^T \sum_{i=1}^{m} \sum_{b=1}^B w_{ib}(t) \, \E_{p\sim \hat{P}_t}\left[ S_{\alpha, i}(p, Y_t) - S_{\alpha, i}(f_b(t), Y_t) \right] + 2\left(\sqrt{\frac{\log mB}{T}}\right) \\
