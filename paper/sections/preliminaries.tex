...
\begin{defi}[Proper scoring rule]\label{def:proper_scoring_rule}
...
Throughout the paper, unless otherwise specified, we take $\mathcal{F}$ to be the class of all probability measures on $(\mathbb{R}, \mathcal{B}(\mathbb{R}))$ or discretized version thereof. Consistency ensures that the expected loss is minimized when the point forecast coincides with the target functional (e.g., the true quantile), thereby extending the idea of propriety from distributions to point-valued predictions. 

\paragraph{Notation.}
Throughout the paper, we say that a scoring function $S$ is \emph{consistent for the quantile level $\alpha$}, or \emph{$\alpha$-quantile-consistent}, if it is consistent for the $\alpha$-quantile functional, $q_\alpha$. Similarly, we say that $S$ is \emph{consistent for multiple quantile levels $\{\tau_n\}_{n=1}^N$} if it is consistent for the vector-valued functional $T = (q_{\tau_1}, \ldots, q_{\tau_N})$. In this case, the action domain is $D_1 = \mathbb{R}^N$. When the quantile levels $\{\tau_n\}_{n=1}^N$ are clear from context, we simply say that $S$ is \emph{consistent for multiple quantile levels}.

\paragraph{Example.} Pinball loss $\rho_\tau(x,y) = \tau |x-y|\indi{x \geq y} + (1-\tau) |x-y|\indi{x < y}$ is strictly consistent scoring function for the $\tau$-quantile functional.



%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\subsection{Omniprediction and two-player game based omniprediction algorithm}
Introduced by \cite{gopalan2022omnipredictors}, \emph{omniprediction} studies the problem of constructing predictors that perform well simultaneously across a family of scoring (loss) functions, relative to a class of comparators. 
% This framework is particularly natural in settings with heterogeneous users, each associated with their own utility (or loss) function.
...
One of the approaches to obtaining an omnipredictor $\hat{p}$ that minimizes the omniprediction objective \eqref{eq:omni_objective} for a finite set of scoring functions $\cs$ is to use a two-player game-based algorithm \cite{garg2024oracle, isaac2025omni, lu2025omni_nonlinear}. One player constructs a weighted sum of loss functions in $\cl$, and the other player responds with the best (randomized) predictor that minimizes the score. Algorithm \ref{alg:omni-twoplayer} shows the details of the algorithm.

\begin{algorithm}
    \caption{Two-player game-based omniprediction} \label{alg:omni-twoplayer}
    \begin{algorithmic}[1]
    \State \textbf{Data:} Data $\{Y_t\}_{t=1}^T \in \cy$, learning rate $\eta > 0$, a class of comparator functions $\cf$, loss functions $\cs = \{S_i\}_{i=1}^m$
...
%     \caption{Two-player game based omniprediction} \label{alg:omni-twoplayer}
%     \begin{algorithmic}[1]
%     \State \textbf{Data:} Data $\{(X_i, Y_i)\}_{i=1}^n$, hyperparameters $m \in \mathbb{N}$, $\eta > 0$, comparator functions $\{\hat{f}_{\theta_i}\}_{i=0}^{m-1}$.
...
% Introduced by \cite{gopalan2022omnipredictors}, omniprediction describes the task of constructing predictors that minimize multiple loss functions simultaneously, compared to a set of comparator functions. Consider a case where there's a diverse set of users with different loss functions, and we want our predictor to minimize the loss for all of them. Let $\cl$ be a set of users' loss functions, and $\cf$ be a set of comparator functions.
...
% Then, given a set of losses $\cl$ and comparator functions $\cf$, omniprediction aims to minimize
% \[
