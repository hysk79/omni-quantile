...
\subsection{Solution for joint minimax program}
...
Due to Corollary 4.2 in \cite{fissler2016multiq}, a loss function that is consistent for multiple quantile levels can be written as a weighted sum of consistent (proper) loss for each quantile levels, under mild regularity conditions. We first consider a loss function that is consistent for two quantile levels $\alpha$ and $\beta$.
\begin{align*}
  S(x,y) = \int_{-\infty}^{\infty} S_{\alpha,\theta}(x,y) dH_\alpha(\theta) + \int_{-\infty}^{\infty} S_{\beta,\theta}(x,y) dH_\beta(\theta),
\end{align*}
where $S_{\tau, \theta} = (\one(y<x)-\tau)(\one(\theta<x)-\one(\theta<y))$ for $\tau = \alpha, \beta$ as defined previously.
...
Now we extend the result to the case of arbitrary number of quantile levels. We denote $N$ as the number of quantile levels and $\tau_1 < \tau_2 < ... < \tau_N$ as the quantile levels. We also denote $\hat{f}_i^n$ for $i =0,...,m\!-\!1$ and $n \in [N]$, which is a $\tau_n$-level quantile forecast. The minimax program is given by

\[
\min_{\tilde{P} \in \cd(\Delta_m)} \max_{P_y \in \Delta_m} \sum_{n=1}^N \sum_{i=0}^{m-1} w_i^n \E_{P\sim \tilde{P}, Y \sim P_y}[S_{\tau_n,\theta_i}(q_{\tau_n}(P),Y) - S_{\tau_n,\theta_i}(\hat{f}_i^n,Y)].
\]

We rewrite the program as follows:
\[
\min_{P_1, P_2, ..., P_N \in \Delta_m} \max_{P_y \in \Delta_m} \sum_{n=1}^N \sum_{i=0}^{m-1} w_i^n \E_{p_n \sim P_
, Y \sim P_y}[S_{\tau_n,\theta_i}(p_n,Y) - S_{\tau_n,\theta_i}(\hat{f}_i^n,Y)],
\]
subject to the constraint $p_n \leq p_{n+1}$ for $n = 1, ..., N-1$, where $S_{\tau_n,\theta_i}(x, Y) = (\one(Y < x)-\tau_n)(\one(\theta_i<x)-\one(\theta_i<Y))$ as defined previously.
...
\subsection{Sample efficient algorithm for omniprediction of multiple quantile level forecasts}\label{subsec:omni_multi_q}
Straightforward extension, to be added. Both algorithm and guarantee.