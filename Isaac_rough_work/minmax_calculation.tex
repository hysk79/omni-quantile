...
\newcommand{\ql}{\textbf{QL-opt}}
\newcommand{\pb}{\textbf{Pinball-opt}}
...
j_{n+1}^* &= \argmin_{j \in [m]} \left( \sum_{i=j}^{m-1} \sum_{s=1}^{{\color{red} n}} w_i^s \indi{\theta_i < \hat{f}_i^s} + \sum_{i=0}^{j-1} \sum_{s=n+2}^{N}  w_i^s (1 - \indi{\theta_i < \hat{f}_i^s}) + \sum_{i=j}^{m-1} w_i^{n+1}\indi{\theta_i < \hat{f}_i^{n+1}}\right).
\end{align*}
